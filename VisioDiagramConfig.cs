namespace BlazorGraph
{
    public class VisioDiagramConfig
    {
        ...
        public double MaxCardHeight { get; set; } = 4;
        public double EffectivePageWidth => PageWidth - HorizontalPageMargin;
        public double EffectivePageHeight => PageHeight - VerticalPageMargin;
        ...
        {
            get => _cardWidth ??= Math.Min(MaxCardWidth, (EffectivePageWidth - (CardsPerRow - 1) * HorizontalMargin) / CardsPerRow);
        }

        public double CardHeight
        {
            get => _cardHeight ??= Math.Min(MaxCardHeight, (EffectivePageHeight - (RowsPerPage - 1) * VerticalMargin) / RowsPerPage);
        }
